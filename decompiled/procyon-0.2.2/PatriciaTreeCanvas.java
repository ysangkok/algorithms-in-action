...
{
    private static final long serialVersionUID = 2210531931154424145L;
    ...
    public void displayAlgorithm(final Graphics g) {
        ...
        if (this.searchPositionNode != null) {
            this.searchPositionNode.draw(g);
            final Point start3;
            start = (start3 = new Point(this.getSize().width / 4, g.getFontMetrics().getHeight() + this.PatriciaTree.getPosition().y));
            this.drawLine(g, start3, new Point(this.searchPositionNode.getX(), this.searchPositionNode.getY()), Messages.getString("PatriciaTreeCanvas.2"));
        }
        if (this.compKeyNode != null) {
            ...
            this.drawLine(g, start4, new Point(this.compKeyNode.getX(), this.compKeyNode.getY()), Messages.getString("PatriciaTreeCanvas.3"));
        }
        if (this.keyFoundNode != null) {
            ...
            this.drawLine(g, start5, new Point(this.keyFoundNode.getX() + this.keyFoundNode.getWidth(), this.keyFoundNode.getY()), Messages.getString("PatriciaTreeCanvas.4"));
        }
        ...
        if (this.patriciaNode != null) {
            ...
            final String diffBitLabel = Messages.getString("PatriciaTreeCanvas.5");
            final Point start6;
            ...
            final String keyValueLabel = Messages.getString("PatriciaTreeCanvas.6");
            final Point start7;
        ...
        if (this.insertPositionNode != null && this.insertPositionNode.getPosition() != null) {
            ...
            this.drawLine(g, start8, new Point(this.insertPositionNode.getBody().getX(), this.insertPositionNode.getBody().getY()), Messages.getString("PatriciaTreeCanvas.7"));
            final Point test = new Point(this.insertPositionNode.getPosition().x, this.insertPositionNode.getPosition().y - 10);
            this.drawLine(g, start, test, Messages.getString("PatriciaTreeCanvas.8"));
        }
        if (this.differentiatingBit != null) {
            final Node node;
            final Node diffBitNode = node = new Node(this.differentiatingBit, 0);
            node.addMarker(new StringMarker(Messages.getString("PatriciaTreeCanvas.9")));
            if (this.PatriciaTree != null) {
        ...
        if (this.currentBit != null) {
            ...
            node2.addMarker(new StringMarker(Messages.getString("PatriciaTreeCanvas.10")));
            if (this.PatriciaTree != null) {
        ...
        if (PatriciaTreeCanvas.upLinkNode != null) {
            PatriciaTreeCanvas.upLinkNode.draw(g);
            g.drawString(Messages.getString("PatriciaTreeCanvas.11"), PatriciaTreeCanvas.upLinkNode.getPosition().x - g.getFontMetrics().stringWidth(Messages.getString("PatriciaTreeCanvas.12")) / 2 + PatriciaTreeCanvas.upLinkNode.getWidth() / 2, PatriciaTreeCanvas.upLinkNode.getPosition().y);
        }
    }
    ...
    public void drawLoops(final Graphics g) {
        if (this.allEndNodes != null) {
            for (int i = 0; i < this.allEndNodes.size(); ++i) {
                final Node[] endNodes = (Node[])this.allEndNodes.elementAt(i);
                final int EDGEGAP = 20;
                final int nodeWidth = endNodes[0].getWidth() + 20;
                final int nodeHeight = endNodes[0].getHeight();
                final Point point;
                ...
                Point endPoint;
                if (endNodes[1] != null) {
                    isThick = (this.isHighlightLoop != null && this.lastNode == endNodes[0] && this.isHighlightLoop == true);
                    final Point point3;
                ...
                if (endNodes[2] != null) {
                    isThick = (this.isHighlightLoop != null && this.lastNode == endNodes[0] && !this.isHighlightLoop);
                    final Point point5;
    ...
    protected synchronized MultipleTween getMoveTweens() {
        ...
        if (this.newNode != null && this.newNode.getHierarchyTree() != null && !this.newNode.hasBeenTweened) {
            ...
            PatriciaTreeCanvas.numberOfTweens = PatriciaTreeCanvas.numberOfTweens + 1;
            multipleTweens.add(new MoveTween(null, this.newNode, this.newNode.getPosition(), this.newNode.getHierarchyTree().getPosition(), false, this.numberOfTweenSteps));
    ...
    public void processRepaintEvent(final RepaintEvent e) {
        if (e.paramObj != null) {
            final PatriciaTree patriciaAlgorithm = (PatriciaTree)((PatriciaTree)e.paramObj);
            ...
            if (this.dataItemBitArray != null) {
                ...
                this.dataItemBitArray.setLabel(PatriciaTreeCanvas.DATA_BIT_ARRAY_LABEL);
            ...
            if (patriciaAlgorithm.getIsFollowedUpLink() != null) {
                if (patriciaAlgorithm.getIsFollowedUpLink().booleanValue()) {
                    PatriciaTreeCanvas.upLinkNode = new Node(Messages.getString("PatriciaTreeCanvas.13"), 0);
                    PatriciaTreeCanvas.upLinkNode.setBackgroundColor(PatriciaTreeColors.YES_COLOR);
                    PatriciaTreeCanvas.upLinkNode.setPosition(upLinkNodePosition);
                }
                else {
                    PatriciaTreeCanvas.upLinkNode = new Node(Messages.getString("PatriciaTreeCanvas.14"), 0);
                    PatriciaTreeCanvas.upLinkNode.setBackgroundColor(PatriciaTreeColors.NO_COLOR);
                    PatriciaTreeCanvas.upLinkNode.setPosition(upLinkNodePosition);
                }
            }
            else {
                PatriciaTreeCanvas.upLinkNode = new Node("?", 0);
                PatriciaTreeCanvas.upLinkNode.setBackgroundColor(PatriciaTreeColors.DEFAULT_BACKGROUND_COLOR);
                PatriciaTreeCanvas.upLinkNode.setPosition(upLinkNodePosition);
            }
            ...
            if (this.compareDataBitArray != null) {
                ...
                this.compareDataBitArray.setLabel(PatriciaTreeCanvas.COMPARE_BIT_ARRAY_LABEL);
            ...
            if (this.PatriciaTree != null) {
                ...
                if (this.windowTop > PatriciaTreeCanvas.PatriciaTreeYPosition) {
                    PatriciaTreeCanvas.PatriciaTreeYPosition = this.windowTop;
                }
                this.PatriciaTree.plantTree(windowCenterX, PatriciaTreeCanvas.PatriciaTreeYPosition);
                ...
                this.windowTop = PatriciaTreeCanvas.PatriciaTreeYPosition + this.PatriciaTree.getRectangle().height;
            }
            ...
            final PatriciaNode temp = patriciaAlgorithm.getNewNode();
            if (temp != null) {
            ...
            if (PatriciaTreeCanvas.numberOfTweens > 0) {
    ...
    static {
        ...
        DATA_BIT_ARRAY_LABEL = Messages.getString("PatriciaTreeCanvas.0");
        COMPARE_BIT_ARRAY_LABEL = Messages.getString("PatriciaTreeCanvas.1");
        PatriciaTreeCanvas.numberOfTweens = 0;
    }
}
