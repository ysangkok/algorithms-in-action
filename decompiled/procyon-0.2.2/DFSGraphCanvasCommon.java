...
public class DFSGraphCanvasCommon extends GraphCanvasCommon
{
    ...
    DFSGraphCanvasCommon(final DFSGraphMapsCanvas p_canvas, final DFSGraphMaps p_algorithm) {
        super();
        this.GRAPH_X = 30;
        this.GRAPH_Y = 30;
        this.vecNodes = null;
    ...
    public void adjustCanvasSize() {
        ...
        for (int i = 0; i < this.vecNodes.size(); ++i) {
            ...
            final DFSGraphMapsCanvas canvas = this.m_canvas;
            if (x + 20 > max_graph_width) {
                final int x2 = node.getX();
                final DFSGraphMapsCanvas canvas2 = this.m_canvas;
                max_graph_width = x2 + 20;
            }
            final int y = node.getY();
            final DFSGraphMapsCanvas canvas3 = this.m_canvas;
            if (y + 20 > max_graph_height) {
                final int y2 = node.getY();
                final DFSGraphMapsCanvas canvas4 = this.m_canvas;
    ...
    public void processDialogEvent(final String sourceDialog, final String value) {
        if (this.m_algorithm != null && !this.m_algorithm.getIsRunning() && value != null) {
            ...
            else if (sourceDialog.equals(Messages.getString("DFSGraphCanvasCommon.1"))) {
            ...
            for (int i = 0; i < this.vecNodes.size(); ++i) {
                ...
                if (px > node.getX() && px < node.getX() + node.getWidth() && py > node.getY() && py < node.getY() + node.getHeight()) {
                    if (this.m_algorithm.getKeyChangeMode()) {
                        ...
                        this.m_dlgKeyChange = new InputDialog(this.m_canvas, this.m_frmKeyChange, Messages.getString("DFSGraphCanvasCommon.2"), true, Messages.getString("DFSGraphCanvasCommon.3"), node.getIntValue());
                        ...
                        switch (this.m_algorithm.getEditLinkStep()) {
                            ...
                            case 2: {
                                ...
                                if (this.m_algorithm.needPromptForLinkWeight()) {
                                    ...
                                    this.m_dlgEditLink = new InputDialog(this.m_canvas, this.m_frmEditLink, Messages.getString("DFSGraphCanvasCommon.4"), true, Messages.getString("DFSGraphCanvasCommon.5"), this.m_algorithm.getCurrentLinkLinkWeight());
    ...
    public void drawLinks(final Graphics g) {
        ...
        for (int i = 0; i < this.vecLinks.size(); ++i) {
            ...
            final DFSGraphMapsCanvas canvas5 = this.m_canvas;
            final int x4 = x3 + 20 / 2 + this.GRAPH_X;
            final int y3 = start.getY();
            final DFSGraphMapsCanvas canvas6 = this.m_canvas;
            final int y4 = y3 + 20 / 2 + this.GRAPH_Y;
            final int x5 = end.getX();
            final DFSGraphMapsCanvas canvas7 = this.m_canvas;
            final int x6 = x5 + 20 / 2 + this.GRAPH_X;
            final int y5 = end.getY();
            final DFSGraphMapsCanvas canvas8 = this.m_canvas;
