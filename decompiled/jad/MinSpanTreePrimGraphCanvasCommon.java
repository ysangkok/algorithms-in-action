...
// Source File Name:   MinSpanTreePrimGraphCanvasCommon.java
...
public class MinSpanTreePrimGraphCanvasCommon
{
    ...
    MinSpanTreePrimGraphCanvasCommon(GraphMapsCanvas p_canvas, GraphMaps p_algorithm)
    {
        GRAPH_X = 30;
        GRAPH_Y = 30;
        vecNodes = null;
    ...
    public void processDialogEvent(String sourceDialog, String value)
    {
        if(m_algorithm != null && !m_algorithm.getIsRunning() && value != null)
            ...
            if(sourceDialog.equals(Messages.getString("MinSpanTreePrimGraphCanvasCommon.1")))
            {
            ...
            for(int i = 0; i < vecNodes.size(); i++)
            {
                ...
                if(px > node.getX() && px < node.getX() + node.getWidth() && py > node.getY() && py < node.getY() + node.getHeight())
                {
                    if(m_algorithm.getKeyChangeMode())
                    {
                        ...
                        m_dlgKeyChange = new InputDialog(m_canvas, m_frmKeyChange, Messages.getString("MinSpanTreePrimGraphCanvasCommon.2"), true, Messages.getString("MinSpanTreePrimGraphCanvasCommon.3"), node.getIntValue());
                        m_dlgKeyChange.show();
                    } else
                    ...
                    if(m_algorithm.editLinkStarted())
                        ...
                        case 2: // '\002'
                            ...
                            if(m_algorithm.needPromptForLinkWeight())
                            {
                                ...
                                m_dlgEditLink = new InputDialog(m_canvas, m_frmEditLink, Messages.getString("MinSpanTreePrimGraphCanvasCommon.4"), true, Messages.getString("MinSpanTreePrimGraphCanvasCommon.5"), m_algorithm.getCurrentLinkLinkWeight());
                                m_dlgEditLink.show();
                            }
                            break;
                        }
