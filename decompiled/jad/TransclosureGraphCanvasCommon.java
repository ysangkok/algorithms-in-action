...
// Source File Name:   TransclosureGraphCanvasCommon.java
...
public class TransclosureGraphCanvasCommon extends GraphCanvasCommon
{

    TransclosureGraphCanvasCommon(GraphMapsCanvas p_canvas, GraphMaps p_algorithm)
    {
        GRAPH_X = 30;
        GRAPH_Y = 30;
        vecNodes = null;
    ...
    public void processDialogEvent(String sourceDialog, String value)
    {
        if(m_algorithm != null && !m_algorithm.getIsRunning() && value != null)
            if(sourceDialog.equals(Messages.getString("TransclosureGraphCanvasCommon.0")))
            {
                m_algorithm.keyChange(m_KeyChangeNode.getIntValue(), Integer.parseInt(value));
                m_algorithm.setKeyChangeMode(false);
            } else
            if(sourceDialog.equals(Messages.getString("TransclosureGraphCanvasCommon.1")))
            {
            ...
            for(int i = 0; i < vecNodes.size(); i++)
            {
                ...
                if(px > node.getX() && px < node.getX() + node.getWidth() && py > node.getY() && py < node.getY() + node.getHeight())
                {
                    if(m_algorithm.getKeyChangeMode())
                    {
                        ...
                        m_dlgKeyChange = new InputDialog(m_canvas, m_frmKeyChange, Messages.getString("TransclosureGraphCanvasCommon.2"), true, Messages.getString("TransclosureGraphCanvasCommon.3"), node.getIntValue());
                        m_dlgKeyChange.show();
                    } else
                    ...
                    if(m_algorithm.editLinkStarted())
                        ...
                        case 2: // '\002'
                            ...
                            if(m_algorithm.needPromptForLinkWeight())
                            {
                                ...
                                m_dlgEditLink = new InputDialog(m_canvas, m_frmEditLink, Messages.getString("TransclosureGraphCanvasCommon.4"), true, Messages.getString("TransclosureGraphCanvasCommon.5"), m_algorithm.getCurrentLinkLinkWeight());
                                m_dlgEditLink.show();
                            }
                            break;
                        }
    ...
    public void drawLinks(Graphics g)
    {
        ...
        for(int i = 0; i < vecLinks.size(); i++)
        {
            ...
            NodeInfo start = link.getStart();
            NodeInfo end = link.getEnd();
            GraphMapsCanvas _tmp4 = m_canvas;
            GraphMapsCanvas _tmp5 = m_canvas;
            GraphMapsCanvas _tmp6 = m_canvas;
            GraphMapsCanvas _tmp7 = m_canvas;
            Line line = new Line(start.getX() + 20 / 2 + GRAPH_X, start.getY() + 20 / 2 + GRAPH_Y, end.getX() + 20 / 2 + GRAPH_X, end.getY() + 20 / 2 + GRAPH_Y);
