...
// Source File Name:   MSTKGraphCanvasCommon.java
...
public class MSTKGraphCanvasCommon
{
    ...
    MSTKGraphCanvasCommon(MSTKGraphMapsCanvas p_canvas, GraphMaps p_algorithm)
    {
        GRAPH_X = 30;
        GRAPH_Y = 30;
        vecNodes = null;
    ...
    public void adjustCanvasSize()
    {
        ...
        {
            GraphMapsNode node = (GraphMapsNode)vecNodes.elementAt(i);
            MSTKGraphMapsCanvas _tmp = m_canvas;
            if(node.getX() + 20 > max_graph_width)
            {
                MSTKGraphMapsCanvas _tmp1 = m_canvas;
                max_graph_width = node.getX() + 20;
            }
            MSTKGraphMapsCanvas _tmp2 = m_canvas;
            if(node.getY() + 20 > max_graph_height)
            {
                MSTKGraphMapsCanvas _tmp3 = m_canvas;
    ...
    public void processDialogEvent(String sourceDialog, String value)
    {
        if(m_algorithm != null && !m_algorithm.getIsRunning() && value != null)
            ...
            if(sourceDialog.equals(Messages.getString("MSTKGraphCanvasCommon.1")))
            {
            ...
            for(int i = 0; i < vecNodes.size(); i++)
            {
                ...
                if(px > node.getX() && px < node.getX() + node.getWidth() && py > node.getY() && py < node.getY() + node.getHeight())
                {
                    if(m_algorithm.getKeyChangeMode())
                    {
                        ...
                        m_dlgKeyChange = new InputDialog(m_canvas, m_frmKeyChange, Messages.getString("MSTKGraphCanvasCommon.2"), true, Messages.getString("MSTKGraphCanvasCommon.3"), node.getIntValue());
                        m_dlgKeyChange.show();
                    } else
                    ...
                    if(m_algorithm.editLinkStarted())
                        ...
                        case 2: // '\002'
                            ...
                            if(m_algorithm.needPromptForLinkWeight())
                            {
                                ...
                                m_dlgEditLink = new InputDialog(m_canvas, m_frmEditLink, Messages.getString("MSTKGraphCanvasCommon.4"), true, Messages.getString("MSTKGraphCanvasCommon.5"), m_algorithm.getCurrentLinkLinkWeight());
                                m_dlgEditLink.show();
                            }
                            break;
                        }
    ...
    public void drawLinks(Graphics g)
    {
        ...
        for(int i = 0; i < vecLinks.size(); i++)
        {
            ...
            NodeInfo start = link.getStart();
            NodeInfo end = link.getEnd();
            MSTKGraphMapsCanvas _tmp4 = m_canvas;
            MSTKGraphMapsCanvas _tmp5 = m_canvas;
            MSTKGraphMapsCanvas _tmp6 = m_canvas;
            MSTKGraphMapsCanvas _tmp7 = m_canvas;
            Line line = new Line(start.getX() + 20 / 2 + GRAPH_X, start.getY() + 20 / 2 + GRAPH_Y, end.getX() + 20 / 2 + GRAPH_X, end.getY() + 20 / 2 + GRAPH_Y);
