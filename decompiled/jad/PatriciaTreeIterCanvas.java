...
public class PatriciaTreeIterCanvas extends AlgorithmCanvas
{
        ...
        implements Moveable
    {
        ...
        NewNode(PatriciaTreeIterNode patriciaNode, Point position)
        {
    ...
    public void displayAlgorithm(Graphics g)
    {
        ...
        if(insertionPoint != null)
        {
            ...
            drawLine(g, start, new Point(insertionPoint.getX(), insertionPoint.getY()), Messages.getString("PatriciaTreeIterCanvas.2"));
        }
        ...
        if(compKeyNode != null)
        {
            compKeyNode.draw(g);
            Point start = new Point(getSize().width / 4, g.getFontMetrics().getHeight() + PatriciaTree.getPosition().y);
            drawLine(g, start, new Point(compKeyNode.getX(), compKeyNode.getY()), Messages.getString("PatriciaTreeIterCanvas.3"));
        }
        if(keyFoundNode != null)
        {
            keyFoundNode.draw(g);
            Point start = new Point((3 * getSize().width) / 4, g.getFontMetrics().getHeight() + PatriciaTree.getPosition().y);
            drawLine(g, start, new Point(keyFoundNode.getX() + keyFoundNode.getWidth(), keyFoundNode.getY()), Messages.getString("PatriciaTreeIterCanvas.4"));
        }
        ...
        {
            patriciaNode.draw(g);
            String diffBitLabel = Messages.getString("PatriciaTreeIterCanvas.5");
            ...
            String keyValueLabel = Messages.getString("PatriciaTreeIterCanvas.6");
        ...
        if(insertPositionNode != null && insertPositionNode.getPosition() != null)
        {
            ...
            drawLine(g, start, new Point(insertPositionNode.getBody().getX(), insertPositionNode.getBody().getY()), Messages.getString("PatriciaTreeIterCanvas.7"));
            Point test = new Point(insertPositionNode.getPosition().x, insertPositionNode.getPosition().y - 10);
            drawLine(g, start, test, Messages.getString("PatriciaTreeIterCanvas.8"));
        }
        if(differentiatingBit != null)
        {
            Node diffBitNode = new Node(differentiatingBit, 0);
            diffBitNode.addMarker(new StringMarker(Messages.getString("PatriciaTreeIterCanvas.9")));
            if(PatriciaTree != null)
        ...
        if(currentBit != null)
        {
            ...
            currentBitNode.addMarker(new StringMarker(Messages.getString("PatriciaTreeIterCanvas.10")));
            if(PatriciaTree != null)
        ...
        if(newNode != null && !newNode.hasBeenTweened)
            newNode.drawNewNode(g);
        if(upLinkNode != null)
        {
            upLinkNode.draw(g);
            g.drawString(Messages.getString("PatriciaTreeIterCanvas.11"), (upLinkNode.getPosition().x - g.getFontMetrics().stringWidth(Messages.getString("PatriciaTreeIterCanvas.12")) / 2) + upLinkNode.getWidth() / 2, upLinkNode.getPosition().y);
        }
    }
    ...
    protected synchronized MultipleTween getMoveTweens()
    {
        ...
        if(newNode != null && newNode.getHierarchyTree() != null && !newNode.hasBeenTweened)
        {
            ...
            numberOfTweens++;
            multipleTweens.add(new InsertTween(null, growNode.getBody(), numberOfTweenSteps));
            multipleTweens.add(new InsertTween(null, growNode.getDataItem().getNode(), numberOfTweenSteps));
    ...
    public void drawLoops(Graphics g)
    {
        if(allEndNodes != null)
        {
            ...
            {
                Node endNodes[] = (Node[])(Node[])allEndNodes.elementAt(i);
                int EDGEGAP = 20;
                int nodeWidth = endNodes[0].getWidth() + 20;
                int nodeHeight = endNodes[0].getHeight() + 6;
                ...
                if(endNodes[1] != null)
                {
                    if(isHighlightLoop != null && lastNode == endNodes[1] && isHighlightLoop.booleanValue())
                        isThick = true;
                    else
                        isThick = false;
                ...
                if(endNodes[2] == null)
                    continue;
                if(isHighlightLoop != null && lastNode == endNodes[2] && !isHighlightLoop.booleanValue())
                    isThick = true;
                else
                    isThick = false;
    ...
    public void processRepaintEvent(RepaintEvent e)
    {
        if(e.paramObj != null)
        {
            PatriciaTreeIter patriciaAlgorithm = (PatriciaTreeIter)(PatriciaTreeIter)e.paramObj;
            int gapWidth = 25;
            windowTop = gapWidth;
            ...
            if(patriciaAlgorithm.getIsFollowedUpLink() != null)
            {
                ...
                {
                    upLinkNode = new Node(Messages.getString("PatriciaTreeIterCanvas.13"), 0);
                    upLinkNode.setBackgroundColor(PatriciaTreeIterColors.YES_COLOR);
                    upLinkNode.setPosition(upLinkNodePosition);
                } else
                {
                    upLinkNode = new Node(Messages.getString("PatriciaTreeIterCanvas.14"), 0);
                    upLinkNode.setBackgroundColor(PatriciaTreeIterColors.NO_COLOR);
                    upLinkNode.setPosition(upLinkNodePosition);
                }
            } else
            {
                upLinkNode = new Node(Messages.getString("PatriciaTreeIterCanvas.15"), 0);
                upLinkNode.setBackgroundColor(PatriciaTreeIterColors.DEFAULT_BACKGROUND_COLOR);
                upLinkNode.setPosition(upLinkNodePosition);
            }
            ...
            insertionPoint = patriciaAlgorithm.getInsertionPoint();
            ...
            PatriciaTreeIterNode temp = patriciaAlgorithm.getNewNode();
            if(temp != null)
            {
                growNode = temp;
    ...
    private static final String DATA_BIT_ARRAY_LABEL = Messages.getString("PatriciaTreeIterCanvas.0");
    private static final String COMPARE_BIT_ARRAY_LABEL = Messages.getString("PatriciaTreeIterCanvas.1");
