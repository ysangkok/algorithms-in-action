...
// Source File Name:   RadixTrieIterCanvas.java
...
public class RadixTrieIterCanvas extends AlgorithmCanvas
{
    ...
    public void displayAlgorithm(Graphics g)
    {
        try
        {
            ...
            if(levelNode != null)
            {
                ...
                levelNode.setPosition(new Point(g.getFontMetrics().stringWidth(Messages.getString("RadixTrieIterCanvas.1")) + 20, windowBottom - 15));
                levelNode.draw(g);
            }
            if(insertData != null)
                insertData.draw(g);
            ...
            if(currentNode != null && !(head instanceof RadixTrieIterNullNode))
            {
                ...
                drawLine(g, start, end, Messages.getString("RadixTrieIterCanvas.2"));
            }
    ...
    public void processRepaintEvent(RepaintEvent e)
    {
        if(e.paramObj != null)
        {
            ...
            RadixTrieIter radixTrieAlgorithm = (RadixTrieIter)(RadixTrieIter)e.paramObj;
            mostSignificantBit = radixTrieAlgorithm.getMostSignificantBit();
            currentLevel = radixTrieAlgorithm.getCurrentLevel();
            if(currentLevel >= 0)
                levelNode = new Node(new Integer(currentLevel), 0);
            else
                levelNode = null;
            ...
            if(insertBitArray != null)
            {
                insertBitArray.setLabel(Messages.getString("RadixTrieIterCanvas.4"));
            ...
            if(compareBitArray != null)
            {
                compareBitArray.setLabel(Messages.getString("RadixTrieIterCanvas.5"));
            ...
            if(searchBitArray != null)
            {
                searchBitArray.setLabel(Messages.getString("RadixTrieIterCanvas.6"));
            ...
            if(finalCompareBitArray != null)
            {
                finalCompareBitArray.setLabel(Messages.getString("RadixTrieIterCanvas.7"));
    ...
    private void drawInsertedData(Graphics g, RadixTrieIterNode node, Point pos)
    {
        if(insertedData != null)
        {
            for(int i = 0; i < insertedData.size(); i++)
            {
                RadixTrieIterExternalNode radixNode = (RadixTrieIterExternalNode)insertedData.elementAt(i);
