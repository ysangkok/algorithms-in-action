...
// Source File Name:   DFSGraphCanvasCommonExt.java
...
public class DFSGraphCanvasCommonExt
{
    ...
    DFSGraphCanvasCommonExt(DFSGraphMapsCanvasExt p_canvas, DFSGraphMaps p_algorithm)
    {
    ...
    public void setMatrix(Vector p_matrix, int matrix_x, int matrix_y)
    {
        ...
        if(vecAdjacencyMatrix != null)
        {
            ...
            DFSGraphMapsCanvasExt _tmp = m_canvas;
    ...
    private void calculateStructureHeight()
    {
        ...
        for(int i = 0; i < m_algorithm.getNumberOfNodes(); i++)
        {
            ...
            do
            {
                if(root == null)
                    break;
                DFSGraphMapsCanvasExt _tmp1 = m_canvas;
    ...
    {
        if(m_algorithm != null && !m_algorithm.getIsRunning() && value != null && sourceDialog.equals(Messages.getString("DFSGraphCanvasCommonExt.0")))
    ...
    public void editMatrix(GraphMapsNode p_Node)
    {
        if(m_algorithm != null && !m_algorithm.getIsRunning())
        {
            ...
            m_dlgMatrixEdit = new InputDialog(m_canvas, m_frmMatrixEdit, Messages.getString("DFSGraphCanvasCommonExt.1"), true, Messages.getString("DFSGraphCanvasCommonExt.2"), p_Node.getIntValue());
    ...
    public void drawAdjacencyMatrix(Graphics g)
    {
        ...
        g.drawString(Messages.getString("DFSGraphCanvasCommonExt.3"), MATRIX_X, MATRIX_Y - fm.getHeight());
        DFSGraphMapsCanvasExt _tmp = m_canvas;
        g.drawString(Messages.getString("DFSGraphCanvasCommonExt.4"), MATRIX_X - fm.stringWidth(Messages.getString("DFSGraphCanvasCommonExt.5")), (MATRIX_Y + 20) - fm.getMaxDescent());
        g.drawString(Messages.getString("DFSGraphCanvasCommonExt.6"), MATRIX_X, MATRIX_Y - fm.getHeight() * 2);
        g.setColor(m_canvas.getTextColor());
        for(int i = 0; i < vecAdjacencyMatrix.size(); i++)
        {
            String label = Integer.toString(i);
            DFSGraphMapsCanvasExt _tmp1 = m_canvas;
            DFSGraphMapsCanvasExt _tmp2 = m_canvas;
            g.drawString(label, MATRIX_X + 20 * i + (20 - fm.stringWidth(label)) / 2, MATRIX_Y - fm.getMaxDescent());
            DFSGraphMapsCanvasExt _tmp3 = m_canvas;
    ...
    public void drawAdjacencyStructure(Graphics g)
    {
        ...
        g.setColor(Color.blue);
        FontMetrics fm = g.getFontMetrics();
        g.drawString(Messages.getString("DFSGraphCanvasCommonExt.7"), earrAdjacencyStructure.getLocation().x, STRUCTURE_Y - fm.getHeight() * 2);
        g.setColor(m_canvas.getTextColor());
...
label0:
        for(int i = 0; i < m_algorithm.getNumberOfNodes(); i++)
        {
            ...
            do
            {
                ...
                DFSGraphMapsCanvasExt _tmp2 = m_canvas;
                DFSGraphMapsCanvasExt _tmp3 = m_canvas;
                DFSGraphMapsCanvasExt _tmp4 = m_canvas;
                ...
                DFSGraphMapsCanvasExt _tmp5 = m_canvas;
